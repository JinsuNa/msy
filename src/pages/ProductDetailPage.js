...
import { ChevronLeft, Heart, ShoppingCart, Share2, MessageCircle, Star } from "lucide-react"
import { Button } from "../components/ui/Button"
import Layout from "../components/Layout"
import RecommendedSection from "../components/RecommendedSection"

// 백엔드 개발자 참고: GET /api/products/:id API 필요
// 응답 형식: { product: { id, name, price, originalPrice, discount, description, images, specifications, reviews, rating } }
function ProductDetailPage() {
  ...
  const addToCart = () => {
    // 실제 구현에서는 Axios를 사용하여 API 호출
    // axios.post('/api/cart/add', { productId: parseInt(id), quantity })
    //   .then(response => {
    //     // 성공 처리
    //   })
    //   .catch(error => {
  ...
  // 즐겨찾기 토글 함수
  // 백엔드 개발자 참고: POST /api/favorites/toggle API 필요
  // 요청 형식: { productId: number }
  // 응답 형식: { success: boolean, isFavorite: boolean }
  const toggleFavorite = () => {
    // 실제 구현에서는 Axios를 사용하여 API 호출
    // axios.post('/api/favorites/toggle', { productId: parseInt(id) })
    //   .then(response => {
        ...
        <div className="bg-white rounded-lg p-4 mb-4">
          ...
          <div className="flex items-center mb-4">
            ...
            <div className="flex items-center border rounded">
              <button className="px-3 py-1" onClick={() => handleQuantityChange(-1)} disabled={quantity <= 1}>
                -
              </button>
          ...
          <div className="flex gap-2 mb-4">
            <Button className="flex-1 bg-blue-500 hover:bg-blue-600 text-white" onClick={addToCart}>
              <ShoppingCart className="h-4 w-4 mr-1" />
              장바구니
            </Button>
            <Button variant="outline" className="p-2" onClick={toggleFavorite}>
          ...
          <div className="flex border-b">
            <button
              className={`flex-1 py-3 text-center ${
                activeTab === "description" ? "border-b-2 border-blue-500 font-medium" : ""
              }`}
            ...
            <button
              className={`flex-1 py-3 text-center ${
                activeTab === "reviews" ? "border-b-2 border-blue-500 font-medium" : ""
              }`}
            ...
            <button
              className={`flex-1 py-3 text-center ${
                activeTab === "qna" ? "border-b-2 border-blue-500 font-medium" : ""
              }`}
